...
const ToastItem = ({ id, type, message, onClose, duration = DEFAULT_DURATION }) => {
  ...
  const getToastClasses = () => {
    const baseClasses = "flex items-center p-4 mb-3 rounded-lg shadow-lg backdrop-blur-md border animate-slide-in";
  ...
  return (
    <div 
      className={`${getToastClasses()} ${isVisible ? 'opacity-100' : 'opacity-0 translate-x-full'} transition-all duration-300 ease-in-out relative`}
      role="alert"
    >
      {getIcon()}
