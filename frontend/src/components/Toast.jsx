...
const ToastItem = ({ id, type, message, onClose, duration = DEFAULT_DURATION }) => {
  ...
  const getToastClasses = () => {
    const baseClasses = "fixed flex items-center p-3 sm:p-4 mb-3 rounded-lg shadow-lg backdrop-blur-md border animate-slide-in";
  ...
  const getIcon = () => {
    switch (type) {
      case TOAST_TYPES.SUCCESS:
        return (
          <div className="inline-flex items-center justify-center flex-shrink-0 w-6 h-6 sm:w-8 sm:h-8 rounded-lg bg-green-500/20">
      ...
      case TOAST_TYPES.ERROR:
        return (
          <div className="inline-flex items-center justify-center flex-shrink-0 w-6 h-6 sm:w-8 sm:h-8 rounded-lg bg-red-500/20">
            <svg className="w-4 h-4 sm:w-5 sm:h-5" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
      ...
      case TOAST_TYPES.INFO:
        return (
          <div className="inline-flex items-center justify-center flex-shrink-0 w-6 h-6 sm:w-8 sm:h-8 rounded-lg bg-blue-500/20">
            <svg className="w-4 h-4 sm:w-5 sm:h-5" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
      ...
      case TOAST_TYPES.WARNING:
        return (
          <div className="inline-flex items-center justify-center flex-shrink-0 w-6 h-6 sm:w-8 sm:h-8 rounded-lg bg-yellow-500/20">
            <svg className="w-4 h-4 sm:w-5 sm:h-5" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
  ...
  return (
    ...
    >
      {getIcon()}
      <div className="ml-2 sm:ml-3 text-xs sm:text-sm font-medium pr-6 sm:pr-8 break-words">{message}</div>
      <button
        type="button"
        className="ml-auto -mx-1.5 -my-1.5 rounded-lg p-1 sm:p-1.5 inline-flex items-center justify-center h-6 w-6 sm:h-8 sm:w-8 hover:bg-white/10 transition-colors flex-shrink-0"
        aria-label="Close"
        onClick={handleClose}
      >
        <span className="sr-only">Close</span>
        <svg className="w-2.5 h-2.5 sm:w-3 sm:h-3" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
          <path fillRule="evenodd" d="M4.293 4.293a1 1 0 011.414 0L10 8.586l4.293-4.293a1 1 0 111.414 1.414L11.414 10l4.293 4.293a1 1 0 01-1.414 1.414L10 11.414l-4.293 4.293a1 1 0 01-1.414-1.414L8.586 10 4.293 5.707a1 1 0 010-1.414z" clipRule="evenodd"></path>
        </svg>
      </button>
...
const ToastContainer = ({ toasts, position = 'bottom-right', onRemoveToast }) => {
  ...
  const getPositionClasses = () => {
    switch (position) {
      case 'top-right':
        return 'top-3 sm:top-4 right-3 sm:right-4';
      case 'top-left':
        return 'top-3 sm:top-4 left-3 sm:left-4';
      case 'bottom-left':
        return 'bottom-3 sm:bottom-4 left-3 sm:left-4';
      case 'top-center':
        return 'top-3 sm:top-4 left-1/2 transform -translate-x-1/2';
      case 'bottom-center':
        return 'bottom-3 sm:bottom-4 left-1/2 transform -translate-x-1/2';
      case 'bottom-right':
      default:
        return 'bottom-3 sm:bottom-4 right-3 sm:right-4';
    }
  };

  return (
    <div className={`fixed z-50 flex flex-col gap-2 sm:gap-3 max-w-[calc(100vw-1.5rem)] sm:max-w-xs w-full ${getPositionClasses()}`}>
      {toasts.map((toast) => (
        <ToastItem
          key={toast.id}
          id={toast.id}
